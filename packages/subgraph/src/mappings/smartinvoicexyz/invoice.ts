...
export function handleUpdatedClient(event: UpdatedClientEvent): void {
  ...
  if (invoice != null) {
    ...
    invoice.client = event.params.client;
    invoice.save();
  }
}
...
export function handleUpdatedProvider(event: UpdatedProviderEvent): void {
  ...
  if (invoice != null) {
    invoice = updateInvoice(event.address, invoice);
    invoice.provider = event.params.provider;
    invoice.save();
  }
}
...
export function handleUpdatedProviderReceiver(event: UpdatedProviderReceiverEvent): void {
  ...
  if (invoice != null) {
    invoice = updateInvoice(event.address, invoice);
    invoice.providerReceiver = event.params.providerReceiver;
    invoice.save();
  }
}
...
export function handleLock(event: LockEvent): void {
  ...
  if (invoice != null) {
    ...
    dispute.ipfsHash = event.params.details;
...
export function handleResolve(event: ResolveEvent): void {
  ...
  if (invoice != null) {
    ...
    resolution.ipfsHash = event.params.details;
